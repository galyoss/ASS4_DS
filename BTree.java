...
public class BTree<T extends Comparable<T>> {
    ...
    // Default to 2-3 Tree
    private int minChildrenSize = 2; // t = 2
    private int minKeySize = minChildrenSize - 1; // t-1 = 1
    private int maxChildrenSize = 2 * minChildrenSize; // 2t = 4
    private int maxKeySize = maxChildrenSize - 1; // 2t-1 = 3
    ...
    public BTree(int order) {
        this.minChildrenSize = order; // t
        this.minKeySize = minChildrenSize - 1; // t-1
        this.maxChildrenSize = 2 * minChildrenSize; // 2t
        this.maxKeySize = maxChildrenSize - 1; // 2t-1
    }
    ...
    public boolean insert(T value) {
        ...
        checkNsplit(root); // at the beging the root have 0 children therefore it won't be splited as needed
        Node<T> current = root;
    ...
    // check and spliting a recived node if necessary, output boolean spliting occured
    private boolean checkNsplit(Node<T> node)
    {
        if (node.numberOfKeys() == maxKeySize) // NOTE: has change after field values uptadet acording to class
        {
            split(node);
            return true;
        }
    ...
    public T delete(T value) {
        T removed = null;
        Node<T> node = this.SearchNodeDel(value);
        if (node.numberOfChildren()==0) // node is leaf. numOfKeys > t-1 , we can delete
        {
        ...
        else //node is internal, numOfkeys > t-1
        {
            ...
            for (int i=0;i<node.numberOfKeys();i++)
                if (node.getKey(i).compareTo(value) == 0)
                    indtodel=i;
            Node prednode = getGreatestNode(node.getChild(indtodel));
            T pred = (T) prednode.getKey(prednode.numberOfKeys()-1);
            removed = node.removeKey(value);
            node.addKey(pred);
            this.remove(pred, prednode);
            return removed;
        }
    ...
    // using for 1-pass deletion. searches for node containing value, while going down the tree
    // making sure invarient of t-1 is being held
    private Node SearchNodeDel(T value)
    {
        Node<T> node = root;
        while (node != null) {
            ...
            if (value.compareTo(greater) > 0) {
                if (node.numberOfChildren() > numberOfKeys) //we have the greatest node, progress to him
                {
                    checkNmerge(node.getChild(node.numberOfChildren()-1));
                    node = node.getChild(node.numberOfChildren()-1);
                }
                else //value we search is greater then any node in the tree
                    node = null;
                continue;
            }
                ...
                if (next <= last) {
                    ...
                    if (currentValue.compareTo(value) < 0 && nextValue.compareTo(value) > 0) {
                        if (next < node.numberOfChildren()) {
                            //TODO : ido to improve efficiency, check specific places, not the while node (after combined)
                            boolean merged = checkNmerge(node.getChild(next));
                            if (!merged) node = node.getChild(next);
                            break;
                        }
                        return null;
                    }
    ...
    //using for 1-pass deletion. if node has min num of keys, the function will fix node so it will have more
    //than min key
    private boolean checkNmerge(Node node)
    {
    ...
    public boolean add(T value) {
        if (root == null) {
            ...
            Node<T> node = root;
            while (node != null) {
                if (node.numberOfChildren() == 0) { //is leaf
                    node.addKey(value);
    ...
    private T remove(T value, Node<T> node) {
        ...
        if (node.numberOfChildren() == 0) {
            ...
            Node<T> greatest = this.getGreatestNode(lesser); // get predessesor node
            T replaceValue = this.removeGreatestValue(greatest); // removes predesser from its node and returns it
    ...
    //TODO: configure left before right
